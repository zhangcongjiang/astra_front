<template>
    <div class="music-list-container">
        ...
        >
          ...
          <template #name="{ record }">
            <div class="music-name-container">
              <span class="music-name-text">{{ record.name }}</span>
              <span class="play-icon-wrapper" @click.stop="togglePlay(record)">
                <span class="play-icon" :class="{ 'playing': currentPlaying.id === record.id && isPlaying }">
      ...
      <a-modal
        ...
        :title="currentMusic ? '编辑音乐信息' : '添加音乐'"
        width="800px"
        :maskClosable="false"
        @ok="handleEditSubmit"
        ...
        >
          ...
          <a-form-item label="音乐标签">
            <TagSearch 
              ...
              :image-tags="musicForm.tags"
              @add-image-tag="addMusicTag"
              @remove-image-tag="removeMusicTag"
            />
          </a-form-item>
        </a-form>
      </a-modal>
